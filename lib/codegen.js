...
const generateFile = options => new Promise((resolve, reject) => {
  ...
  fs.readFile(path.resolve(root, file_name), 'utf8', (err, content) => {
    ...
    try {
      ...
      let output_content;
      if (getFileExtension(file_name) === 'js') {
        output_content = beautify(parsed_content);
      } else {
        output_content = parsed_content;
      }
      fs.writeFile(generated_path, output_content, 'utf8', (err) => {
        if (err) return reject(err);
        resolve();
      });
    } catch (e) {
...
const generateOperationFile = (config, operation, operation_name) => new Promise((resolve, reject) => {
  ...
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    ...
    const ne_filename = config.file_name.replace('___', operation_name);
    const new_filename = ne_filename.substr(0, ne_filename.length - 4);
    ...
    fs.writeFile(target_file, beautify(content), 'utf8', (err) => {
      if (err) return reject(err);
      resolve();
    });
  });
...
const generateDirectoryStructure = config => new Promise((resolve, reject) => {
  ...
  xfs.mkdirpSync(path.resolve(target_dir, './src/schema'));
  openapi2schema(config.pureSwagger, null, function(err, res) {
    if (err) return console.error(err);
    fs.writeFile(path.resolve(target_dir, `./src/schema/schemas.json`), JSON.stringify(res, null, 2), 'utf8', (err) => {
      if (err) return reject(err);
      resolve();
    });
  });
 ...
 * @return {Promise}
 */
export const codegen = config => new Promise((resolve, reject) => {
  swagger2(config.swagger).then(swagger => {
    const random_name = randomName().dashed;
    config.pureSwagger = config.swagger;
    ...
    _.defaultsDeep(config, {
      ...
      templates: path.resolve(__dirname, '../templates/fastify-server')
    });
